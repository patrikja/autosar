...
import qualified System.IO           as Sys ( stdin, hGetContents )
import qualified System.Environment  as Sys ( getArgs, getProgName )
import qualified System.Exit         as Sys ( exitFailure, exitSuccess )

import qualified Eprover.Par   as P
import qualified Eprover.Print as P
import qualified Eprover.Lex   as L
import qualified Eprover.ErrM  as E
import Eprover.Abs
import Data.Char
...
----------------------------------------------------------------
type ParseFun a = [L.Token] -> E.Err a
type Verbosity = Int
...
putStrLnV v s = if v > 1 then putStrLn s else return ()

runFile :: (Print a, Show a) => Verbosity -> ParseFun a -> FilePath -> IO ()
runFile v p f = putStrLnV v f >>
                readFile f >>= run v p
...
usage = do
  putStrLn $ unlines
    ...
    , "  -s (files)      Silent mode (default). Parse content of files silently."
    , "  -v (files)      Verbose mode."
    ]
  Sys.exitFailure
...
main = do
  ...
  case args of
    ["--help"]  -> usage
    []          -> Sys.hGetContents Sys.stdin >>= run 2 P.pDatabase
    "-s":fs     -> mapM_ (runFile 0 P.pDatabase) fs
    "-v":fs     -> mapM_ (runFile 2 P.pDatabase) fs
    fs          -> mapM_ (runFile 0 P.pDatabase) fs
