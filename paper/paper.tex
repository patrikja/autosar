...
Common state for runnable \V{R} of component \V{I} with continuation \V{K}, showing \V{N} current instances, \V{T} seconds left until next possible instantiation, and activation state $\Act$.
...
Exclusive area \V{X} of component \V{I}, with boolean occupation state \V{B}.
...
Non-queued sender-receiver data element \V{E} in port \V{P} of component \V{I}, currently holding value \V{V} with boolean update status flag \V{B}.
...
Our semantic approach defines the meaning of an AUTOSAR system as the possible chains of state changes that can be applied to the system's initial state; or equivalently, as the set of \emph{traces} that can be generated from the corresponding process term \V{P_0}. A trace is a possibly infinite sequence of transitions
\[
\begin{array}{c}
...
The label \V{L} of a transition can be of two kinds, indicating that the affected process term either "says" or "hears" something during the transition. A label also always consists of an address \V{A} paired with some additional detail \V{D}, as captured in the following label grammar:
\[
\begin{array}{rcll}
...
\begin{mathpar}
  \inferrule
    {\V{P} \red{\V{A}!\V{D}} \V{P'} \\ \V{Q} \red{\V{A}?\V{D}} \V{Q'}}
    {\Tpar{\V{P}}{\V{Q}} \red{\V{A}!\V{D}} \Tpar{\V{P'}}{\V{Q'}}}
\and
  \inferrule
    {\V{P} \red{\V{A}?\V{D}} \V{P'} \\ \V{Q} \red{\V{A}!\V{D}} \V{Q'}}
    {\Tpar{\V{P}}{\V{Q}} \red{\V{A}!\V{D}} \Tpar{\V{P'}}{\V{Q'}}}
...
\item For each runnable \V{R} of each component \V{I} with continuation (code) \V{K}, a term
\[
\begin{array}{c}
...
\end{array}
\]
where \V{V} is the initial value attributed to \V{S} if it exists, otherwise \V{V} is $\Tvoid$.
...
\]
if there is a connector in the model from provided sender-receiver port \V{P_1} of component \V{I_1} to required port \V{P_2} of component \V{I_2}; and
\[
  \Tconnect{\Top{\V{O}}{\V{P_1}}{\V{I_1}}}{\Top{\V{O}}{\V{P_2}}{\V{I_2}}} \;\;\text{for all operations \V{O} of port \V{P_1}}
\]
if there is a connector in the model from required client-server port \V{P_1} of component \V{I_1} to provided port \V{P_2} of component \V{I_2}.
