...
\end{array}
$$
are derivable, we note that process $p$ may evolve into state $q$ either via an $l_1$ step followed by $l_2$, or by first taking the $l_2$ step and then $l_1$. Such freedom of choice is commonly taken as the definition of {\em parallelism} in the process calculus literature --- if the sequential ordering of steps $l_1$ and $l_2$ has no significance, they might just as well be considered to occur in parallel.
...
The AUTOSAR specification contains a very subtle sensitivity to this kind of non-deterministism that is worth examplifying. Consider a queued data element $a$ together with a runnable $b$ set up to trigger on data reception events for $a$. A scenario where two values $v_1$ and $v_2$ are sent on a port connected to $a$ may lead to the following trace:
$$
\begin{array}{c}
...
\item Category 2 runnables, which declare certain read or receive operations as {\em WaitPoints} that block progress until the associated events occur. While this form of event handling is fundamentally different from the standard mechansim of activating runnables, it may be noted that the result command that follows from calling a {\em synchronousServerCallPoint} already defines the essence of such a blocking {\em WaitPoint}.
\item The AUTOSAR COM layer and related events and commands ({\em DataWriteCompletedEvent}, {\em DataSendCompletedEvent}, {\em DataReceiveErrorEvent}, the feedback command) \cite{AR:COM}. COM is the communication substrate between nodes in a distributed AUTOSAR system and its limited storage capacity introduces an element of resource competition among communicating components. It also leads to the separation of sending and reception into distinct events, as well as a new class of communication errors.
\item Most details found in the {\em ComSpec} annotations that can be attached to ports (currently only {\em initValues} are acknowledged). 
