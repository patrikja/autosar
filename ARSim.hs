...
runAR                   :: (forall c. AR c a) -> State -> (a, [P], [Conn])
...
say m (ENTER _) (RInst a c ex (Enter x cont))           = [RInst a c (x:ex) (cont void)]
say m (EXIT _) (RInst a c (_:ex) (Exit x cont))         = [RInst a c ex (cont void)]
say m (IRVR _ res) (RInst a c ex (IrvRead _ cont))      = [RInst a c ex (cont res)]
say m (IRVW _ _) (RInst a c ex (IrvWrite _ _ cont))     = [RInst a c ex (cont void)]
