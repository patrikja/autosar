...
minim lim       = map (\(t,v) -> (1,if v < lim then lim else v))
...
test1 = do
        ...
        v_sensors <- mapM input ["v1","v2","v3","v4"]
        a_sensors <- mapM input ["a1","a2","a3","a4"]
        connectAll v_sensors velos_in
        connectAll a_sensors accels_r_in
        connectAll a_sensors accels_p_in
        
        r_actuators <- mapM output ["r1","r2","r3","r4"]
        p_actuators <- mapM output ["p1","p2","p3","p4"]
        connectAll valves_r_out r_actuators
        connectAll valves_p_out p_actuators
        
        environment say hear (v1,v2,v3,v4,a1,a2,a3,a4)
        
say (v1,v2,v3,v4,a1,a2,a3,a4)
  | t == 0                     = (Data a v, gen)
  | t > 0                      = (Time t, (t',a,v):gen)

hear (Data a v) (t,gen) = (t, gen)
hear (Time d)   (t,gen) = (t+d, gen)
