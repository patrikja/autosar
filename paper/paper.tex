...
\item Category 2 runnables, which declare certain read or receive operations as {\em WaitPoints} that block progress until the associated events occur. While this form of event handling is fundamentally different from the standard mechansim of activating runnables, it may be noted that the implicit result command that follows from calling a {\em synchronousServerCallPoint} already defines the essence of such a blocking {\em WaitPoint}.
...
\item COM layer
\item DataWriteCompletedEvent, DataSendCompletedEvent, DataReceiveErrorEvent, the feedback command
\item RTE lifecycle and callback APIs
\end{itemize}
